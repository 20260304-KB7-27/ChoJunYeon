...
const updatePostHandler = (v) => {
  const targetPost = posts.value.find((post) => post.no === v.no);
  if (targetPost) {
    targetPost.title = v.title;
    targetPost.content = v.content;
  }
};

const deletePostHandler = (v) => {
...
<template>
  <div id="app">
    ...
    <!-- 아래의 목록을 완성하세요 -->
    <InputTodo @addPost="addPostHandler"></InputTodo>
    <TodoList
      :posts="posts"
      @passUpdatedPost="updatePostHandler"
